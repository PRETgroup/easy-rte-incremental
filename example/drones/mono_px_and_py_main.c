...
#define RUN_REFERENCE "mono_px_and_py"
...
int main() {
    ...
    for (uint16_t run = 0; run < RUNS; run++) {
        ...
        start = clock();
        enforcervars_px_and_py_t enf;
        inputs_px_and_py_t inputs;
        outputs_px_and_py_t outputs;
        
        px_and_py_init_all_vars(&enf, &inputs, &outputs);
        ...
        while(count++ < TICKS_PER_RUN) {
            ...
            px_and_py_run_via_enforcer(&enf, &inputs, &outputs);
...
void px_and_py_run(inputs_px_and_py_t *inputs, outputs_px_and_py_t *outputs) {
    //do nothing
}
