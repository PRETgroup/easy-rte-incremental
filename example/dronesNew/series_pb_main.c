#include "series_F_pb.h"
#include <stdio.h>

#include "save_results.h"
#include <time.h>
#define RUN_REFERENCE "series_pb"

void print_data(uint32_t count, inputs_pb_t inputs, outputs_pb_t outputs) {
...
int main() {
    ...
    for (uint16_t run = 0; run < RUNS; run++) {
        ...
        start = clock();
        enforcervars_pb_t enf;
        inputs_pb_t inputs;
        outputs_pb_t outputs;
        
        pb_init_all_vars(&enf, &inputs, &outputs);
        ...
        while(count++ < TICKS_PER_RUN) {
            ...
            pb_run_via_enforcer(&enf, &inputs, &outputs);
...
void pb_run(inputs_pb_t *inputs, outputs_pb_t *outputs) {
    //do nothing
}
