#include "F_pb_and_ps.h"
#include <stdio.h>

#include "save_results.h"
#include <time.h>
#define RUN_REFERENCE "monolithic_pb_and_ps"

void print_data(uint32_t count, inputs_pb_and_ps_t inputs, outputs_pb_and_ps_t outputs) {
    printf("Tick %7d: x:%d, x_up:%d, x_down:%d, y:%d, y_up:%d, y_down:%d, rpm:%d, rpm_up:%d, rpm_down:%d\r\n", count, inputs.x, outputs.x_up, outputs.x_down, inputs.y, outputs.y_up, outputs.y_down, inputs.rpm, outputs.rpm_up, outputs.rpm_down);
}
...
int main() {
    ...
    for (uint16_t run = 0; run < RUNS; run++) {
        ...
        start = clock();
        enforcervars_pb_and_ps_t enf;
        inputs_pb_and_ps_t inputs;
        outputs_pb_and_ps_t outputs;
        
        pb_and_ps_init_all_vars(&enf, &inputs, &outputs);
...
void pb_and_ps_run(inputs_pb_and_ps_t *inputs, outputs_pb_and_ps_t *outputs) {
    //do nothing
}
